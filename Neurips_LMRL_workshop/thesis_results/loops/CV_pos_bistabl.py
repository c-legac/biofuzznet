...
for fold in range(args.folds):
    ...
    G_sim.edges()[("B", "C")]["layer"].n = torch.nn.Parameter(torch.tensor(0.6080123477547604))
    G_sim.edges()[("B", "C")]["layer"].K = torch.nn.Parameter(torch.tensor(-1.0433492801100686))
    G_sim.edges()[("C", "OR")]["layer"].n = torch.nn.Parameter(torch.tensor(0.499241513702521))
    G_sim.edges()[("C", "OR")]["layer"].K = torch.nn.Parameter(torch.tensor(-0.44126596346853575))
    G_sim.edges()[("A", "OR")]["layer"].n = torch.nn.Parameter(torch.tensor(1.5853397866217094))
    G_sim.edges()[("A", "OR")]["layer"].K = torch.nn.Parameter(torch.tensor(0.1954952050293041))
