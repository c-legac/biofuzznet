"""
    This generate the Boolean version
"""
...
for combinations in range(len(cell_inputs)):
    input_dic = {"41BB": torch.tensor(cell_inputs[combinations][0]),
    "CD28":  torch.tensor(cell_inputs[combinations][1]),
    "CD40":  torch.tensor(cell_inputs[combinations][2]),
    "CTLA4":  torch.tensor(cell_inputs[combinations][3]),
    "IL15Ra":  torch.tensor(cell_inputs[combinations][4]),
    "CD45":  torch.tensor(cell_inputs[combinations][5])
    }
